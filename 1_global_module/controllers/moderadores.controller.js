let Administrador = require("../models/profesores.model") 
        ...
        if(req.currentUser.get("permisoPtr")){
            if(req.currentUser.get("permisoPtr").get("administracion")){
                ...
                && _.has(req.currentUser.get("permisoPtr").get("administracion").administracion.usuarios, "lectura")){
                    res.redirect("/profesores/permisos")   
                }else{
                    log("Usuario sin permiso básico de profesores")
                    res.redirect("/profesores/acceso")
                }
            }else{
                log("Usuario sin permisos de profesores")
                res.redirect("/profesores/acceso")
            }
        }else{
...
module.exports.Acceso = async(req, res) => {
    ...
    if(req.cookies.type  || req.cookies.usr_id){
        ...
        res.render('NovusTec/Profesores/Acceso/index', {
